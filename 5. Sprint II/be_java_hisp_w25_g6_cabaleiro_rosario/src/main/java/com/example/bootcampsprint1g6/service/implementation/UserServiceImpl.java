...
public class UserServiceImpl implements IUserService {
    ...
    public FollowersDTO getFollowersList(Integer userId, String order) {
        ...
        if (order != null) {
            switch (order) {
                case NAME_ASC -> followersList.sort(Comparator.comparing(User::getUserName));
                case NAME_DESC -> followersList.sort(Comparator.comparing(User::getUserName).reversed());
            }
        }
    ...
    private void validateFollow(User buyer, User seller) {
        if (buyer.getFollowed().stream().anyMatch(f -> f.getUserId().equals(seller.getUserId()))) {
    ...
    public FollowedDTO getFollowedList(Integer userId, String order) {
        ...
        if (order != null) {
            switch (order) {
                case NAME_ASC -> followedUsers.sort(Comparator.comparing(User::getUserName));
                case NAME_DESC -> followedUsers.sort(Comparator.comparing(User::getUserName).reversed());
            }
        }
