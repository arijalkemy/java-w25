...
-- 5 Nombre, apellido y velocidad de internet de clientes con paquetes de mas de 300 megas
SELECT clients.first_name, clients.last_name, packs.speed
FROM clients JOIN packs ON clients.pack_id = packs.pack_id
WHERE packs.speed > 300;

-- 6 Promedio de velocidad de internet de clientes de CABA
SELECT AVG(packs.speed) as promedio
FROM clients JOIN packs ON clients.pack_id = packs.pack_id
WHERE clients.city = 'CABA';

-- 7 Mejor pack relación precio/cantidad de megas
SELECT pack_id, speed
FROM packs
ORDER BY speed / price DESC
LIMIT 1;

-- 8 Pack con mayor descuento
SELECT pack_id, speed
FROM packs
ORDER BY discount DESC
LIMIT 1;

-- 9 Nombre y apellido de clientes nacidos en el siglo XXI
SELECT first_name, last_name
FROM clients
WHERE birthday > '2000-01-01';

-- 10 Nombre y apellido de clientes que no nacieron en CABA
SELECT first_name, last_name
FROM clients
WHERE city != 'CABA';