...
public class ObraService implements IObraService {
    ...
    public List<ObraDto> getTop5Pages() {
        List<Obra> obras = obraRepository.findTop5OrderByPaginas();
        return obras
                .stream()
                .map(obra -> mapper.map(obra, ObraDto.class))
                .collect(Collectors.toList());
    ...
    @Override
    public List<ObraDto> findByYearBefore(Integer year) {
        List<Obra> obras = obraRepository.findByAnioBefore(year);
        return obras
                .stream()
                .map(obra -> mapper.map(obra, ObraDto.class))
                .collect(Collectors.toList());
