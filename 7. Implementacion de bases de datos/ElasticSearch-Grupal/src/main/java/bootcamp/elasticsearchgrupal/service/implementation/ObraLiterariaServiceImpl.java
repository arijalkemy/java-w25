...
public class ObraLiterariaServiceImpl implements IObraLiterariaService {
    ...
    public List<ObraLiterariaResponseDTO> getObrasLiterariasTop5OrderedByCantidadDepaginasDesc() {
        return getAllObrasliterarias().stream().sorted(Comparator.comparing(ObraLiterariaResponseDTO::getCantidadDePaginas)).limit(5).toList();
    }

    @Override
    public List<ObraLiterariaResponseDTO> getObrasLiterariasBeforeYear(Integer year) {
        return obraLiterariaRepository.findObraLiterariasByAnioDePrimeraPublicacionIsBefore(year).stream().map(MapperClass::entityToDto).toList();
    }
