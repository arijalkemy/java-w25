...
public class SalesController {
    ...
    public ResponseEntity<SaleDto> getSaleByCode(@PathVariable String number){
        return new ResponseEntity<>(salesService.getSaleById(number), HttpStatus.OK);
    }

    //Actualizar una venta en particular
    @PutMapping("/{number}")
    public ResponseEntity<ConfirmationMessage> updateClothes(@RequestBody SaleReqDto saleDto, @PathVariable String number){
        return new ResponseEntity<>(salesService.updateSale(saleDto, number), HttpStatus.CREATED);
    }
    //Eliminar una venta en particular
    @DeleteMapping("/{number}")
    public ResponseEntity<ConfirmationMessage> deleteSaleByCode(@PathVariable String number){
    ...
    public ResponseEntity<List<ClotheDto>> getClothesBySale(@PathVariable String number){
