...
public class LiteraryWorkService implements ILiteraryWorkService{
    ...
    @Override
    public List<LiteraryWorkResponseDto> getLiteraryWorksByFirstPublishingYearBefore(int year){
        return repository.findLiteraryWorksByFirstPublishingYearBefore(year).stream()
                .map(literaryWork -> mapper.map(literaryWork, LiteraryWorkResponseDto.class))
                .toList();
    }
    @Override
    public List<LiteraryWorkResponseDto> getLiteraryWorksByEditorial(String editorial){
        return repository.findLiteraryWorksByEditorial(editorial).stream()
                .map(literaryWork -> mapper.map(literaryWork, LiteraryWorkResponseDto.class))
                .toList();
    }
}
