...
public class ClothesServiceImpl implements IClothesService{
    ...
    public ConfirmationMessage saveNewClothes(ClotheReqDto clothesDTO) {
        ...
        return new ConfirmationMessage("La prenda fue guardada exitosamente con codigo: " + savedClothe.getId() + ".");
    }
    ...
    @Override
    public ClotheDto getClotheByCode(String code) {
        return mapper.map(clotheRepository.findById(code), ClotheDto.class);
    }

    @Override
    public ConfirmationMessage updateClothe(ClotheReqDto clotheDto, String code) {
        Clothe existingClothe = clotheRepository.findById(code).orElseThrow();
        Clothe newClothe = mapper.map(clotheDto, Clothe.class);
        newClothe.setId(existingClothe.getId());
        return new ConfirmationMessage("La prenda con codigo: " + clotheRepository.save(newClothe).getId() + " fue actualizada exitosamente.");
    }

    @Override
    public ConfirmationMessage deleteClothesById(String code) {
