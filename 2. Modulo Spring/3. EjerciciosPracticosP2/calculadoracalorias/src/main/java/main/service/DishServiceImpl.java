...
public class DishServiceImpl implements IDishService {
    ...
    public FoodResponseDTO comidaCaloriasMaxima(DishRequestDTO dish) {
        ...
        Food maxFood = dishRepository.getByName(name).getFoods().stream().max(Comparator.comparing(food-> Double.parseDouble(food.getCalories())/100.0)).orElse(null);
        return new FoodResponseDTO(maxFood.getName(), String.valueOf(Double.parseDouble(maxFood.getCalories()) * (dish.getWeight())/100.0));
    }
