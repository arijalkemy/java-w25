...
public class LinkController {
     ...
     * */
    @PostMapping("/newLink")
    public ResponseEntity<LinkIdResponseDTO> newLink(
            @RequestBody LinkRequestDTO newLink, @RequestParam(required = false) String password) {
    ...
    public ResponseEntity<ResponseDTO> redirect(
            @PathVariable int linkId, @RequestParam(required = false) String password) {
        RedirectResponseDTO resDTO = linkService.redirect(RedirectMapper.toRedirectRequestDTO(linkId,password));

        HttpHeaders header = new HttpHeaders();
        header.setLocation(URI.create(resDTO.getUrl()));
        return new ResponseEntity<>(new ResponseDTO("Success, redirecting..."), header, HttpStatus.FOUND);
    }
