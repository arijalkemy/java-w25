...
public class UsersServiceImp implements IUsersService {
    ...
    public VendorsFollowingListDto getFollowingList(FollowingListReqDto req) {
        ...
        if (clientFollowing != null)
            return clientFollowing;

        VendorsFollowingListDto vendorFollowing = getVendorsFollowingListDto(order, vendor);
        if (vendorFollowing != null)
            return vendorFollowing;
    ...
    public MessageDto unfollowVendor(UserUnfollowVendorDTO req) {
        ...
        if (userClient != null) {
            // Usé new ArrayList para que no tire excepcion ya que devuelve
        ...
        if (removedFromClient || removedFromVendor) {
            return new MessageDto(
                    "El usuario con id " + userId + " ha dejado de seguir al vendedor con id " + vendorId);
        } else {
            throw new NotFoundException(
                    "Error: El usuario con id " + userId + " no está siguiendo al vendedor con id " + vendorId);
        }
    }
