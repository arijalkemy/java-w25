...
public class PostService implements IPostService {
    ...
    public void addPost(PostDTO postDto) {
        ...
        Post finalPost;
        //Ej11 - modified
        if (postDto.getHas_promo()){
            finalPost = new Post(idCounter.incrementAndGet(),postDto.getHas_promo(), postDto.getDiscount(), postDto);
        }else {
            finalPost = new Post(idCounter.incrementAndGet(), postDto);
        }
    ...
    //***********************************************INDIVIDUAL***********************************************
    @Override
    public PromoCountDto obtainPromosCountByVendorId(Integer userId) {
        User vendor = userRepository.findUserByUserId(userId).orElseThrow(() -> new NotFoundException("No se encontro al vendedor"));
        Integer promosCount = postRepository.getPromosCountByIdVendor(userId);
        return new PromoCountDto(vendor, promosCount);
    }
    @Override
    public PromosListDto obtainPromosByVendorId(Integer id) {
        User vendor = userRepository.findUserByUserId(id)
                                    .orElseThrow(() -> new NotFoundException("No se encontro al vendedor"));
        List<PromoPostDto> promosDto = new ArrayList<>();
        List<Post> promosVendor = postRepository.getPromosByIdVendor(id);
        for (Post p : promosVendor){
            Product promoProduct = productRepository.getProductById(p.getProductId())
                                                    .orElseThrow(() -> new NotFoundException("No se encontro el producto"));
            promosDto.add(new PromoPostDto(p, new ProductDto(promoProduct)));
        }
        return new PromosListDto(vendor, promosDto);
    }


    //Auxiliares
    private void validatePost(PostDTO post) {
        if (post.getUserId() <= 0) throw new InvalidDataException("Error al enviar los datos: Usuario no válido");
        ...
        //Ej 12- validación agregada
        if (post.getHas_promo()) {
            if (post.getDiscount() < 0 || post.getDiscount() > 100)
                throw new InvalidDataException("Producto con promoción con descuento no válido.");
        }

        if (post.getDate() == null) post.setDate(LocalDate.now());
        ...
        if (post.getCategory() <= 0) throw new InvalidDataException("Error al enviar los datos: Categoría no válida");
        if (post.getPrice() <= 0) throw new InvalidDataException("Error al enviar los datos: Precio no válido");
    }
