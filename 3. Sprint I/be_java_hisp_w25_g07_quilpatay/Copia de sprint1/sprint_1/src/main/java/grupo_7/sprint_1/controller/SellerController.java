...
import grupo_7.sprint_1.dtos.SellerPromosListDto;
...
public class SellerController {
    ...
    public ResponseEntity<?> getFollowersList(@PathVariable Integer userId, @RequestParam String order) {
    ...
    public ResponseEntity<?> getRecentPostsFromFollowedSellers(@PathVariable Integer userId, @RequestParam String order) {
    ...
    public ResponseEntity<?> countSellerPromos(@RequestParam(name = "user_id") Integer userId) {
        try {
            SellerPromosListDto sellerDTO = sellerService.getSellerPromosCount(userId);
            return new ResponseEntity<>(sellerDTO, HttpStatus.OK);
        } catch (NotFoundException e) {
            return ResponseEntity.notFound().build();
        }
