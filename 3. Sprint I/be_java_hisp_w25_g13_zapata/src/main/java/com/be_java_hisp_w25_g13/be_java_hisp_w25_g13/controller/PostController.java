...
public class PostController {
    ...
    public ResponseEntity<?> createPromotion(@RequestBody PromoDTO promoDTO){
        return new ResponseEntity<>(postService.addPromotionPost(promoDTO), HttpStatus.OK);
    }

    @GetMapping("/promo-post/count")
    public ResponseEntity<?> getPromos(@RequestParam Integer userId){
        return new ResponseEntity<>(postService.getPromos(userId), HttpStatus.OK);
    }

    @GetMapping("/promo-post/{userId}/list")
    public ResponseEntity<?> getPromosPerUser(@PathVariable Integer userId,
                                              @RequestParam(defaultValue = "none") String order){
        return new ResponseEntity<>(userService.getPromosPerUser(userId, order), HttpStatus.OK);
    }
}
