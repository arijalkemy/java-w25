...
public class PostServiceImpl implements IPostService {
    ...
    public PromoPostCountDto getPromoPostCount(Integer userId) {
        int count = 0;
        var seller  = sellerService.getById(userId);
         count = (int)seller.getPosts().stream()
                .map(postRepository::getById)
                .filter(Post::isHas_promo)
                .count();

        return new PromoPostCountDto(seller.getId(), seller.getName(), count);
    }

    @Override
    public PromoPostDto getPromoPostListById(Integer userId, String order) {
        Seller seller  = sellerService.getById(userId);

        List<PostResponseDto> posts = new ArrayList<>(seller.getPosts().stream()
                .map(postRepository::getById)
                .filter(Post::isHas_promo)
        ...
        if(order != null){
            switch (order) {
                case "name_asc":
                    posts.sort(Comparator.comparing(post -> post.getProduct().getProduct_name()));
                    break;
                case "name_desc":
                    posts.sort(Comparator.comparing(post -> post.getProduct().getProduct_name(), Comparator.reverseOrder()));
        ...
        return new PromoPostDto(seller.getId(), seller.getName(), posts);
    }
}
