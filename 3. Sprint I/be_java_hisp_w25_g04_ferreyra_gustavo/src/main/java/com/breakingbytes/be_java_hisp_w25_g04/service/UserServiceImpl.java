...
public class UserServiceImpl implements IUserService {
    ...
    public void follow(int userId, int userIdToFollow) {
        ...
        if (me.get().getBlocking() != null && me.get().getBlocking().contains(userToFollow.get())) {
            throw new BadRequestException("No puedes seguir a un usuario bloqueado");
        }
    ...
    @Override
    public void block(int userId, int userIdToBlock) {
        Optional<User> me = this.userRepository.findById(userId);
        Optional<Seller> userToBlock = this.sellerRepository.findById(userIdToBlock);

        if (me.isEmpty()) throw new NotFoundException("Ususario no encontrado");
        if (userToBlock.isEmpty()) throw new NotFoundException("Vendedor no encontrado");

        if (me.get().getBlocking().contains(userToBlock.get())) {
            throw new BadRequestException("Ya bloqueaste a ese usuario");
        }

        if (me.get().getFollowing().contains(userToBlock.get()))
            this.unfollowUser(Integer.toString(userId), Integer.toString(userIdToBlock));

        this.userRepository.addBlocking(me.get(), userToBlock.get());
    }
}
