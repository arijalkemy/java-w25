...
public class PostServiceImp implements IPostService {
    ...
    public PostPostPromoCountDto getPostPromoCount(Long userId) {
        Integer count= postRepository.getByIdUser(userId).stream().filter(postDto -> postDto.getHas_promo()!=null&&postDto.getHas_promo()).toList().size();
        Optional<Seller> seller= sellerRepository.findById((int)userId.longValue());
        if (!seller.isEmpty()){
            return new PostPostPromoCountDto(userId,seller.get().getName(),count);
}else{
