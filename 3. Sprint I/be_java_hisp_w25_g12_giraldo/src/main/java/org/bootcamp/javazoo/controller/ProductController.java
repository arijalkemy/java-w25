...
public class ProductController {
    ...
    public ResponseEntity<?> addNewPostPromo(@Valid @RequestBody PostPromoDto postPromoDto, BindingResult bindingResult){
        if(bindingResult.hasErrors()){
            throw new BadRequestException("Invalid fields");
        }
        return new ResponseEntity<>(postService.addNewPostPromo(postPromoDto) , HttpStatus.CREATED);
    }

    @GetMapping("/promo-post/count")
    public ResponseEntity<?> countProductsPromoBySeller(@RequestParam(required = true) int user_id){
        return new ResponseEntity<>(postService.countProductsPromoBySeller(user_id), HttpStatus.OK);
    }

    @GetMapping("/promo-post/list")
    public ResponseEntity<?> listProductsPromoBySeller(@RequestParam(required = true) int user_id, @RequestParam(required = false) String order){
        return new ResponseEntity<>(postService.listProductsPromoBySeller(user_id, order), HttpStatus.OK);
    }
}
