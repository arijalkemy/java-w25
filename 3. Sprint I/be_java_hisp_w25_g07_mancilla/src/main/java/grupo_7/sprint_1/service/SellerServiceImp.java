...
public class SellerServiceImp implements ISellerService {
    ...
    @Override
    public MessageDto addPostPromo(int idUser, PostPromoDto postPromo) {
        Post post = Mapper.convertPostPromoDtoToPost(postPromo);

        Optional<Seller> foundSeller = sellerRepository.findById(idUser);
        if (foundSeller.isEmpty()) {
            throw new NotFoundException("El vendedor indicado no existe: "+idUser);
        }

        Optional<Post> postProductId = foundSeller.get().getPosts().stream()
                .filter(p -> p.getProduct().getProductId().equals(postPromo.product().productId()) && p.getHasPromo())
                .findFirst();

        if(postProductId.isPresent()) {
            throw new InvalidArgsException("Ya existe una promocion del mismo producto, con el mismo 'product_id'."+postPromo.product().productId());
        }

        sellerRepository.addPostPromo(idUser, post);
        return new MessageDto("Se creò correctamente el post con promocion.");
    }

    @Override
    public PostPromoResponseDto countPostPromoByUserId(int idUser) {
        Optional<Seller> seller = sellerRepository.findById(idUser);
        if (seller.isEmpty()) {
            throw new NotFoundException("El vendedor indicado no existe: "+idUser);
        }
        int count = (int) seller.get().getPosts().stream()
                .filter(Post::getHasPromo)
                .count();

        return new PostPromoResponseDto(idUser, seller.get().getUserName(), count);
    }

    @Override
    public PostPromoListDto getAllPromoByIdUser(int userId) {
        Optional<Seller> seller = sellerRepository.findById(userId);
        if (seller.isEmpty()) {
            throw new NotFoundException("El vendedor indicado no existe: "+userId);
        }
        List<Post> posts = seller.get().getPosts();

        List<PostPromoDto> promoList = posts.stream()
                .filter(Post::getHasPromo)
                .map(post -> Mapper.convertPostPromoToDto(seller.get().getUserId(), post))
                .toList();

        return new PostPromoListDto(seller.get().getUserId(), seller.get().getUserName(), promoList);
    }
}
