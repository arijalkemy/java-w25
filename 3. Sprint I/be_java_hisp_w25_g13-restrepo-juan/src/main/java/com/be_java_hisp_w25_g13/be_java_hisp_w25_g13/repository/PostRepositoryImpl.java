...
public class PostRepositoryImpl implements IPostRepository{
    ...
    @Override
    public Integer getPromoPostCount(Integer idUsuario) {
        return posts.stream()
                .filter(post -> post.getUserId().equals(idUsuario) && post.getHasPromo().equals(true))
                .toList()
                .size();
    }

    @Override
    public List<Post> getPromoPostByIdUsuario(Integer idUsuario) {
        return posts.stream()
                .filter(post -> post.getUserId().equals(idUsuario))
                .toList();
    }
}
