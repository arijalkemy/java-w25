...
public class Mapper {
    ...
    public static Post mapPromoPostDtoToPost(PromoPostDTO promoPostDTO){
        return new Post(promoPostDTO.getUserId(), promoPostDTO.getDate(),
                mapProductDtoToProduct(promoPostDTO.getProduct())
    ...
    public static List<PromoPostDTO> postListToPromoPostDTOList(List<Post> posts){
        List<PromoPostDTO> responseList = new ArrayList<>();
        for (Post post:
             posts) {
            responseList.add(
                    new PromoPostDTO(
