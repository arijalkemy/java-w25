...
public class Repository implements IRepository {
    ...
    @Override
    public long sellerDiscountedProductsQuantity(int sellerId) {
        return posts.stream()
        .filter(
            post -> post.getUser().getId() == sellerId 
            && post.getProduct().getDiscount() > 0 
        )
        .count();
    }
    ...
    @Override
    public List<Product> getSellerDiscountedProducts(int sellerId) {
        return posts.stream()
        .filter(
            post -> post.getUser().getId() == sellerId 
            && post.getProduct().getDiscount() > 0 
        ).map(post -> post.getProduct())
        .toList();
    }
}
