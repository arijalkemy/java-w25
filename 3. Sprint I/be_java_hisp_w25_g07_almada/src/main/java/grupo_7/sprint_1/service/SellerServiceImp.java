...
public class SellerServiceImp implements ISellerService {
    ...
    @Override
    public GenericResponseDTO addPromoPost(PromoPostDto promoPostDto) {
        Seller seller = sellerRepository.findById(promoPostDto.userId()).orElseThrow(() -> new NotFoundException("El vendedor con el ID " + promoPostDto.userId() + " no existe"));
        seller.getPosts().stream()
                .filter(post -> post.getProduct().getProductId().equals(promoPostDto.product().productId()))
                .findFirst()
                .ifPresent(post -> {throw new InvalidArgsException("Ya existe un producto con el mismo 'product_id'.");
        });
        Post post = Mapper.convertPromoPostDtoToPromoPost(promoPostDto);
        seller.getPosts().add(post);
        sellerRepository.updateSeller(seller);
        return new GenericResponseDTO("El post se ha creado con éxito");
    }

    @Override
    public CountPromosDto getPromoPost(Integer sellerId) {
        Seller seller = sellerRepository.findById(sellerId).orElseThrow(() -> new NotFoundException("El vendedor con el ID " + sellerId + " no existe"));
        Integer promos = (int) seller.getPosts().stream()
                .filter(post -> post.getDiscount() != null && post.getHasPromo())
                .count();
        return new CountPromosDto(sellerId, seller.getUserName(), promos);
    }
}
