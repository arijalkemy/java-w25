...
public class SellerController {
    ...
    public ResponseEntity<?> addPromoPost(@RequestBody PromoPostDto newPost) {
        return new ResponseEntity<>(sellerService.addPromoPost(newPost), HttpStatus.OK);
    }

    // US 0011: Contar la totalidad de productos con promo de un determinado vendedor
    @GetMapping("/products/promo-post/count")
    public ResponseEntity<?> getPromoPost(@RequestParam ("user_id") Integer userId) {
        return new ResponseEntity<>(sellerService.getPromoPost(userId), HttpStatus.OK);
    }
