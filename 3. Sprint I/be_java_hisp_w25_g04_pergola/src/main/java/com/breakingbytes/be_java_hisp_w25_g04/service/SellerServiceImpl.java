...
public class SellerServiceImpl implements ISellerService{
    ...
    public SellerServiceImpl(ISellerRepository sellerRepository, Mapper mapper, IPostRepository postRepository, IProductRepository productRepository, IUserRepository userRepository) {
    ...
    @Override
    public PromoCountsDTO getCountPromoProds(String sellerId) {
        int sellerIdInt = Integer.valueOf(sellerId);
        Optional<Seller> sellerOpt = sellerRepository.findById(sellerIdInt);
        if(sellerOpt.isEmpty()) throw new NotFoundException("ID de vendedor invalido.");
        Seller seller = sellerOpt.get();
        int promoProdsCounter = (int)seller.getPosts().stream().filter(p -> p.isHas_promo()).count();
        return new PromoCountsDTO(seller.getId(), seller.getName(), promoProdsCounter);
    }

    @Override
    public List<PromoPostsDTO> getPromoPost(int userId) {
        //TODO
        return null;
    }
