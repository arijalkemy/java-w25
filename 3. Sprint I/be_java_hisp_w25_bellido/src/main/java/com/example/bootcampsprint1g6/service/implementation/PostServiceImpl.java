...
public class PostServiceImpl implements IPostService {
    ...
    @Override
    public PromoCountDTO countPromosById(Integer userId) {
        Optional<User> user = userRepository.findById(userId);
        if (user.isEmpty())
            throw new NotFoundException("El usuario con id " + userId + " no existe.");

        Integer postCount = getPostPromoList(userId).size();

        return PromoCountDTO.builder().userId(userId).userName(user.get().getUserName()).productPromoCount(postCount).build();
    }

    @Override
    public PostPromoListDTO listPromosByUserId(Integer userId) {
        Optional<User> user = userRepository.findById(userId);
        if (user.isEmpty())
            throw new NotFoundException("El usuario con id " + userId + " no existe.");

        List<PostPromoResponseDTO> posts = getPostPromoList(userId).stream()
                .map(p -> PostMapper.getResponseInstance((PostPromo) p)).toList();

        return PostPromoListDTO.builder().userId(userId).usernName(user.get().getUserName()).posts(posts).build();
    }

    private List<Post> getPostPromoList(Integer userId) {
        return postRepository.findAll().stream()
                .filter(p -> p.getSeller().getUserId().equals(userId))
                .filter(p -> p.getClass().equals(PostPromo.class)).toList();
    }
