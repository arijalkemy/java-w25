...
public class PostController {
    ...
    public ResponseEntity<PostPromoResponseDTO> createPostPromo(@RequestBody PostPromoRequestDTO request){
        return new ResponseEntity<>(postService.createPostPromo(request),HttpStatus.OK);
    }

    @GetMapping("/promo-post/count")
    public ResponseEntity<PostPromoCountDTO> getPostPromoCount(
        @RequestParam(required = false, name = "user_id") Integer userId
    ){
        return new ResponseEntity<>(postService.getPostPromoCount(userId), HttpStatus.OK);
    }

    @GetMapping("/promo-post/list")
    public ResponseEntity<FullPostPromoResponseDTO> getPostPromoList(
        @RequestParam(required = false, name = "user_id") Integer userId
    ){
