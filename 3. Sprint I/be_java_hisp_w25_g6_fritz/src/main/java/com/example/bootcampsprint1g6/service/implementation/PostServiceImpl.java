...
public class PostServiceImpl implements IPostService {
    ...
    public PostPromoResponseDTO createPostPromo(PostPromoRequestDTO request) {
        ...
        PostPromo postToCreate = PostMapper.getPromoEntityInstance(request, (Seller) user.get());
        Post postCreated = postRepository.save(postToCreate);
        userRepository.addPostToUser(postCreated);
        return PostMapper.getPromoResponseInstance((PostPromo)postCreated);
    }

    @Override
    public PostPromoCountDTO getPostPromoCount(Integer userId) {
        List<Post> posts = postRepository.findAll();
        User user = userRepository.findById(userId).orElseThrow(() -> new NotFoundException("No se encuentran posts con promocion para el usuario con id: "+userId));
        Integer postPromoCount = posts.stream().filter(p -> p.getSeller().equals(user) && ((PostPromo)p).getHasPromo().equals(true)).toList().size();
        return new PostPromoCountDTO(user.getUserId(), user.getUserName(), postPromoCount);
    }

    @Override
    public FullPostPromoResponseDTO getPostPromoList(Integer userId) {
        List<Post> posts = postRepository.findAll();
        User user = userRepository.findById(userId).orElseThrow(() -> new NotFoundException("No se encuentran posts con promocion para el usuario con id: "+userId));
        List<Post> postsPromo = posts.stream().filter(p -> ((PostPromo)p).getHasPromo()).toList();
        return new FullPostPromoResponseDTO(user.getUserId(),user.getUserName(),postsPromo.stream().map(p -> PostMapper.getPromoResponseInstance((PostPromo)p)).toList());
    }
