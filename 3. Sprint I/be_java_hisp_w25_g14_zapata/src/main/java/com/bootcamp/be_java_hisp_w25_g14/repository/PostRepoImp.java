...
public class PostRepoImp implements IPostRepo{
    ...
    @Override
    public List<Post> getPostsOnDiscountByVendor(Integer id) {
        return this.postList.stream().filter(post -> post.getUserId().equals(id) && post.getIsOnDiscount().equals(true)).toList();
    }
