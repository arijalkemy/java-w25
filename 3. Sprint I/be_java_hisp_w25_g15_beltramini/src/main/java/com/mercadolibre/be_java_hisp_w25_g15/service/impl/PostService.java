...
public class PostService implements IPostService {
    ...
    @Override
    public PostGetListDto getPromoPostsByUser(int userId, String order) {
        User user = userRepository.getUserById(userId)
                .orElseThrow(() -> new NotFoundException("User with id " +userId+ "not found."));
        if (!(user instanceof Seller)){
            throw new ConflictException("User is not a seller");
        }
        List<PostDto> postDtoList = this.postRepository.getPostPromoByUser(userId).stream()
                .map(post -> mapper.getMapper().convertValue(post, PostDto.class)).toList();
        if(postDtoList.isEmpty()){
            throw new NotFoundException("The seller with id " +userId+ " haven´t promo posts.");
        }

        return new PostGetListDto(userId, sortPostDtoListByProductName(postDtoList, order));
    }
    ...
    private List<PostDto> sortPostDtoListByProductName(List<PostDto> postListDtos, String order) {

        List<PostDto> newPostList = new ArrayList<>(postListDtos);

        if (order != null) {
            //order asc by product name
            newPostList.sort(Comparator.comparing(postDto -> postDto.product().product_name()));
            if (order.equals("name_desc")) {
                //order desc by product name
                newPostList.sort(Comparator.comparing(postDto -> postDto.product().product_name(), Comparator.reverseOrder()));
            }
        }
        return newPostList;
    }
