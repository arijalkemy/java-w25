...
public class PostController {
    ...
    public ResponseEntity<GenericResponseDTO> getCountPromoPostByUser(@RequestParam(value = "user_id") Integer userId){
        return ResponseEntity.ok(postService.getCountPromoPost(userId));
    }

    @GetMapping("/promo-post/list")
    public ResponseEntity<List<PostResponseDTO>> getListPromoPostByUser(@RequestParam(value = "user_id")Integer userId){
        return ResponseEntity.ok(postService.getListPromoPostByUserId(userId));
    }
}
